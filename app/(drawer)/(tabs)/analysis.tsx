        ...
        {loans.length === 0 ? (
          ...
          <>
            ...
            <Card>
              ...
              <Typography variant="sm" color="slate" style={s.sectionSub}>
                Monthly interest bleeding from each loan
              </Typography>
              {loans.sort((a, b) => b.interestRate - a.interestRate).map(loan => {
                const monthlyInt = loan.outstandingBalance * (loan.interestRate / 12 / 100);
                const pct = totalInterestPerMonth > 0 ? (monthlyInt / totalInterestPerMonth) * 100 : 0;
                return (
                  <View key={loan.id} style={s.leakRow}>
                    <View style={{ flex: 1 }}>
                      <Typography weight="medium" color="navy">{loan.name}</Typography>
                      <Typography variant="sm" color="slate" style={s.leakRate}>
                        {loan.interestRate}% · {formatCurrency(monthlyInt)}/mo
                      </Typography>
                    </View>
                    <View style={s.leakBarBg}>
                      <View style={[s.leakBarFill, {
...
const s = StyleSheet.create({
  ...
  leakRow: { flexDirection: 'row', alignItems: 'center', gap: Spacing.md, paddingVertical: Spacing.md, borderBottomWidth: 1, borderBottomColor: '#f1f5f9' },
  leakRate: { marginTop: 2 },
  leakBarBg: { width: 80, height: 6, borderRadius: 3, backgroundColor: '#f1f5f9', overflow: 'hidden' },
  leakBarFill: { height: 6, borderRadius: 3 },
